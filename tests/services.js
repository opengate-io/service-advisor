...
Test('/services', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         */
        t.test('test getServices get operation', function (t) {
            Mockgen().requests({
                path: '/services',
                operation: 'get'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/services']['get']['responses']['200']['schema']);
         ...
         */
        t.test('test createService post operation', function (t) {
            Mockgen().requests({
                path: '/services',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/services']['post']['responses']['201']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
